...
public class CygwinEnvironment extends GccEnvironment {
  ...
  private void assertCygwinInstalled() {
    ...
    if (!root.exists() || !root.isDirectory()) {
      throw new IllegalStateException("Cygwin root folder does not exist at " + root.getAbsolutePath());
    }
  }

  public File getWorkingDirectory() {
    return new File(cygwinRoot + File.separator + "tmp" + File.separator + tempFolderName);
  }
  ...
  public Process startGcc(List<String> arguments) throws IOException {
    ...
    ProcessBuilder gcc = new ProcessBuilder().command(
        cygwinRoot + File.separator + "bin" + File.separator + "bash.exe", "-c", script.toString()).directory(
        new File(cygwinRoot + File.separator + "bin"));
